...
describe("Update user", () => {
  ...
  describe("submit success", () => {
    beforeEach(async () => {
      ...
      let inputNameValue = await page.$eval(
        "[data-test=input-name]",
        el => (el as HTMLTextAreaElement).value
      );
      // focus on the input field
      await page.click("[data-test=input-name]");
      while (inputNameValue.length > 0) {
        await page.keyboard.press("Backspace");
        inputNameValue = await page.$eval(
          "[data-test=input-name]",
          el => (el as HTMLTextAreaElement).value
        );
      }
      await page.type("[data-test=input-name]", "update");

      const inputEmailValue = await page.$eval(
        "[data-test=input-email]",
        el => (el as HTMLTextAreaElement).value
      );
      // focus on the input field
      await page.click("[data-test=input-email]");
      for (let i = 0; i < inputEmailValue.length; i++) {
        await page.keyboard.press("Backspace");
      }
      await page.type("[data-test=input-email]", "update@example.com");
    });
    test("display updated user show page and dialog message [kwSS1orxFfQCzf3If3uhZ]", async () => {
      await Promise.all([
        page.waitForSelector("[data-test=user-name]"),
        page.click("[data-test=submit]"),
      ]);
    ...
    test("does not change user image when image file is empty [dv_xA1JUd25jcjyw37KGS]", async () => {
      await Promise.all([
        page.waitForSelector("[data-test=user-image]"),
        page.click("[data-test=submit]"),
      ]);
    ...
    test("changes user image when image file is selected [qF2FAE3HCIulZYARTtMkb]", async () => {
      ...
      const filePath = path.join(
        __dirname,
        `../../../backend/public/image/users/${fileName}`
      );
      expect(
